        ...
        switch (msg.topic.toLowerCase()) {
          ...
          case "":
            ...
            if (isNaN(msg.payload)) {
              ...
              if (this.manual_profile) {
                if (this.manual_profile.pre_target === result.target) {
                  result = this.getState(msg.payload, this.manual_profile);
                } else {
                  this.manual_profile = null;
                }
              }
            ...
            break;
          case "settarget":
            var pre_target = this.getState(0, this.profile).target;
            result = setTarget(msg.payload);
            result.profile.pre_target = pre_target;
            if (result.isValid) {
              this.manual_profile = result.profile;
              this.current_status = result.status;
              this.status(this.current_status);
            }
            break;
            ...
            if (result.found) {
              ...
              if (this.profile.name === "default") {
                ...
                this.current_status = {
                  fill:  "green",
                  shape: "dot",
                  text:  "profile set to default (" + this.profile.name + ")"
                };
              } else {
                this.current_status = result.status;
              }
    ...
    if (current > target_plus) {
      ...
      if (state) {
        status = {
          fill:  "yellow",
          shape: "dot",
          text:  target_minus + " < " + current + " < " + target_plus + " (" + profile.name + ")"
        };
      } else {
        status = {
          fill:  "grey",
